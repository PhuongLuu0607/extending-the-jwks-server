...
def private_key_to_pem_pkcs1(priv) -> bytes:
    # Serialize RSA private key sang PKCS#1 PEM để lưu vào SQLite (kiểu BLOB)
...
# --- DB layer (parameterized queries: chống SQLi) ---
class KeyDB:
    def __init__(self, path=DB_FILE):
        # ✅ AUTOCOMMIT để không phải gọi conn.commit() thủ công
    ...
    def insert_key(self, pem: bytes, exp: int) -> int:
        ...
        return cur.lastrowid  # autocommit đã bật
...
# Seed DB: 1 key hết hạn & 1 key còn hạn lần đầu chạy
db = KeyDB()
...
class MyServer(BaseHTTPRequestHandler):
    ...
    # GET /.well-known/jwks.json: trả về JWKs của các key còn hạn
    def do_GET(self):
    ...
    # POST /auth[?expired=1] -> ký JWT bằng key (valid/expired)
    def do_POST(self):
        parsed = urlparse(self.path)
        ...
        # Lấy username: ưu tiên HTTP Basic; fallback JSON body {"username": ...}
        username = "userABC"
